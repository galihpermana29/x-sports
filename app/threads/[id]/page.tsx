...
async function ThreadsDetailPage({ params }: { params: { id: string } }) {
  const paramsId = parseInt(params.id);

  const { data: threadDetail } = await GET.getThreadById(paramsId);
  const { data: hotThreads } = await GET.getAllThreads();
    ...
    <main className="flex flex-col md:flex-row gap-5 max-w-screen-xl mx-auto px-5 py-10 md:px-10">
      ...
      <aside className="basis-[40%] overflow-hidden lg:basis-[30%] w-full h-fit rounded-md bg-xport-black-light">
        ...
        <div className="flex flex-col mt-5">
          {hotThreads?.map(({ id, title, game_names, image_thread }, index) => {
            return (
              id !== paramsId &&
              index < 6 && (
                <Link
